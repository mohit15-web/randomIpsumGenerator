  ...
  const handleClick = () => {
    if(number > 10){
      ...
      for (let i = 1; i <= number; i++) {
        paragraphs.push(<p key={i} className="w-[50%] m-auto text-center">{i+")"}{loremIpsumArray[i]}</p>);
      }
      setNumber("")
  ...
  return (
    <div className="w-[100%] flex justify-center items-center flex-col gap-10 py-10">
      ...
      <div className="flex gap-2 justify-start items-center py-2 px-6">
        ...
        />
        <button className="inline-block py-1 px-4 rounded-md bg-[#F5863C] hover:bg-white hover:text-[#7747FF] focus:text-[#7747FF] focus:bg-gray-200 text-gray-50 font-bold leading-loose transition duration-200" 
        onClick={handleClick}>
          GENERATE
        </button>
      </div>
